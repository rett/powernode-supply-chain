...
module SupplyChain
  # Executes a single ScanExecution server-side: marks it running, dispatches by
  # the scan template's category to the appropriate scanner service, persists the
  # status / output_data / duration, optionally creates an auto-remediation plan,
  # and broadcasts lifecycle events over ActionCable.
  #
  # Extracted verbatim from the worker ScanExecutionJob so the standalone Sidekiq
  # worker can stay HTTP-only (no ActiveRecord). The worker job now POSTs to
  # /api/v1/internal/supply_chain/scan_executions/:id/run, which invokes this.
  #
  # On scanner error the execution is marked failed, a failure is broadcast, and
  # the error is re-raised so the internal endpoint surfaces a 5xx (letting the
  # worker's Sidekiq retry).
  class ScanExecutionRunner
    # @param execution [SupplyChain::ScanExecution]
    ...
    #
    # @return [Hash] the execution's persisted output_data (scan results merged
    #   with findings_count)
    # @raise [StandardError] re-raised after marking the execution failed
    def run!
      # Broadcast start
