...
module SupplyChain
  # Builds a supply-chain Report server-side: aggregates across the supply-chain
  # schema per +report_type+, persists the generated content onto the Report
  # record, and broadcasts the generation lifecycle over ActionCable.
  #
  # Extracted from the worker ReportGenerationJob so the standalone Sidekiq worker
  # can stay HTTP-only (no ActiveRecord). The worker job now POSTs to
  # /api/v1/internal/supply_chain/reports/:id/generate, which invokes this.
  class ReportBuilder
    # @param report [SupplyChain::Report]
    def initialize(report)
      @report = report
    end

    # Generates and persists the report content, driving the report through the
    # generating -> completed lifecycle (or -> failed, re-raising, on error).
    # @return [SupplyChain::Report]
    def build!
      # Broadcast start
